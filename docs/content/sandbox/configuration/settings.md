...
If your configuration has both OLLAMA or OpenAI as providers for chat and embeddings LLMs, it will appear a button named “Download SpringAI” on the bottom left corner of the GUI:

![SpringAI](images/download_springai.png)

Currently mixed configurations, like for example: Ollama for embeddings and OpenAI for chat completion are not allowed, since the function is done to run the chatbot in private or public mode only.

After having downloaded and unzipped the file, you can open and set the latest two things in the code to be executed. For the detailed description, please refer to the **README.md** file included.
