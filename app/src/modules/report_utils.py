...
def record_update():
    ...
    if "reference_context_input" not in st.session_state:
        st.session_state.reference_context_input = st.session_state.df.iloc[st.session_state.index][
            "reference_context"
        ]
    ...
    with col1:
        if st.button("previous"):
            ...
            if st.session_state.index > 0:
                ...
                st.session_state.reference_answer_input = st.session_state.df.at[
                    st.session_state.index, "reference_answer"
                ]
                st.session_state.reference_context_input = st.session_state.df.at[
                    st.session_state.index, "reference_context"
                ]
    ...
    with col3:
        ...
        if save_clicked:
            ...
            st.session_state.df.at[st.session_state.index, "reference_answer"] = (
                st.session_state.reference_answer_input
            )
            st.session_state.df.at[st.session_state.index, "reference_context"] = (
                st.session_state.reference_context_input
            )
    ...
    st.session_state.index_output = st.write(
        "Record: " + str(st.session_state.index + 1) + "/" + str(st.session_state.df.shape[0])
    )
    ...
    st.session_state.reference_answer_input = st.text_area(
        "Reference answer", height=1, value=st.session_state.reference_answer_input
    )
    st.session_state.reference_context_input = st.text_area(
        "Reference context", height=10, value=st.session_state.reference_context_input, disabled=True
    )
    st.session_state.metadata_input = st.text_area(
        "Metadata", height=1, value=st.session_state.metadata_input, disabled=True
    )
